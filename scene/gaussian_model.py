...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args):
        ...
        self.pre_cull = getattr(args, "pre_cull", False)
        self.pre_cull_opacity = float(getattr(args, "pre_cull_opacity", 0.0))
        self.pre_cull_min_radius = float(getattr(args, "pre_cull_min_radius", 0.0))
    ...
    def load_ply(self, path):
        ...
        x_raw = np.asarray(plydata.elements[0]["x"])
        y_raw = np.asarray(plydata.elements[0]["y"])
        z_raw = np.asarray(plydata.elements[0]["z"])
        if np.issubdtype(x_raw.dtype, np.integer):
            bbox_min = None
            bbox_max = None
            bits = None
            if hasattr(plydata, "comments") and plydata.comments:
                for c in plydata.comments:
                    if c.startswith("bbox_min"):
                        bbox_min = np.array([float(v) for v in c.split()[1:4]], dtype=np.float32)
                    elif c.startswith("bbox_max"):
                        bbox_max = np.array([float(v) for v in c.split()[1:4]], dtype=np.float32)
                    elif c.startswith("xyz_quant_bits"):
                        try:
                            bits = int(c.split()[1])
                        except Exception:
                            bits = None
            if bbox_min is None or bbox_max is None:
                ...
                if os.path.exists(meta_path):
                    ...
                    with open(meta_path, "r", encoding="utf-8") as f:
                        meta = json.load(f)
                    bbox_min = np.array(meta.get("bbox_min"), dtype=np.float32)
                    bbox_max = np.array(meta.get("bbox_max"), dtype=np.float32)
                    bits = int(meta.get("bits", 16))
            if bbox_min is None or bbox_max is None:
                raise RuntimeError("Quantized xyz requires bbox_min/bbox_max in PLY comments or .json sidecar")
            if bits is None:
                bits = 16 if x_raw.dtype == np.uint16 else 8
            maxv = float((1 << bits) - 1)
            q = np.stack((x_raw, y_raw, z_raw), axis=1).astype(np.float32)
            xyz = bbox_min + (q / maxv) * (bbox_max - bbox_min)
        else:
